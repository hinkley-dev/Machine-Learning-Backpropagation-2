...
void LayerLinear::backprop(const Vec& weights, Vec& prevBlame)
{
  ...
  //   std::cout << "Blame: " << Blame.rows() << " x " << Blame.cols() << std::endl;
  //   throw Ex("error multiplyig matricies in LL backprop");
  // }

  Matrix* PrevBlame = Matrix::multiply(M, Blame, true, true);

  if(PrevBlame->rows() != 1)
  {
    PrevBlame = PrevBlame->transpose();
  }
  if(PrevBlame->rows() != 1)
    throw Ex("Error computing prevBlame.");
  prevBlame.copy(PrevBlame->row(0));
